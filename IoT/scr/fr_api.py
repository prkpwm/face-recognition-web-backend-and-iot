...
from mongo_lib import (get_checkin_status, get_name_role, get_ontime,
                       get_status_pm25, get_status_thermal, get_status_weather,
                       get_working_day, getStatusFace, insert_log_predict)
from utility import check_weather_pm25, checkTemperature
...
async def predict_mask(request):
    ...
    try:
        ...
        if image_mask != '' and image_mask != None:
            ...
            # prepare data
            img_mask = Image.open(os.path.join(
                os.getcwd(), 'image_no_crop', image_mask)).convert('RGB')
            ...
            if y_hat == 1:
                ...
                maskback_multi = mp.Process(
                    target=thread_var_mask, args=('mask',))
...
async def predict(request):
    ...
    if status_pm25 == True:
        ...
        weather_pm25_mp = mp.Process(
            target=thread_weather, args=(weather_back,))
    ...
    try:
        ...
        if files_name != '' and files_name != None and log_pre != '' and log_pre != None:
            ...
            if is_recognition:
                idx, nickname, firstname, lastname, role, confident, image_name = face_prediction(
                    files_name)
                pred_res.append('nickname: {},firstname: {},lastname: {},role: {},confident: {},imagename: {}'.format(
                    nickname, firstname, lastname, role, confident, image_name))
                print('nickname: {}\nfirstname: {}\nlastname: {}\nrole: {}\nconfident: {}\nimagename: {}'.format(
                    nickname, firstname, lastname, role, confident, image_name))
            ...
            if status_pm25 == True:
                ...
                location_res = weather_back.get()
                location_res = location_res.split(",")[1]
            ...
            if nickname == 'fnf':
                ...
                data = {"detect": False, "nickname": 'fnf', "role_name": 'fnf', "weather_res": weather_res,
                        "pm25_res": pm25_res, "checktime": checktime, "temp": round(temp, 2), "location_res": location_res}
                with open('../../frontend/src/pred.json', 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=4)

                t2socket = mp.Process(target=thread_send_socket, args=(
                    False, 'fnf', 'fnf', weather_res, pm25_res, checktime, temp,))
                t2socket.start()
                ...
                status_log_pre = insert_log_predict(
                    log_pre, idx, files_name, int(temp), pm25_res, weather_res, -1)
                print('insert log predict status:', status_log_pre)
                print('inserted: log_pre:{}, idx:{}, files_name:{}, temp:{}, pm25_res:{}, weather_res:{}'.format(
                    log_pre, idx, files_name, temp, pm25_res, weather_res))
                role_name = get_name_role(role)
                print('role:', role_name)
                # send thread to socket
                import json
                data = {"detect": True, "nickname": nickname, "role_name": role_name, "weather_res": weather_res,
                        "pm25_res": pm25_res, "checktime": checktime, "temp": round(temp, 2), "count": count, "location_res": location_res}
                with open('../../frontend/src/pred.json', 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=4)
                t2socket = mp.Process(target=thread_send_socket, args=(
                    True, nickname, role_name, weather_res, pm25_res, checktime, temp,))
                t2socket.start()
...
routes = [
    Route("/recognition/predict_mask",
          endpoint=predict_mask, methods=["POST"]),
    ...
    Route("/recognition/resulte",
          endpoint=recognition_resulte, methods=["POST"]),

]


def sock():
    socketio.run(app, host='192.168.0.252', port=5432)
    # HOST = '192.168.0.252'  # IP ของ server
...
if __name__ == "__main__":
    # p_process = mp.Process(target=sock)
    # p_process.start()
    data = {"detect": False}
    with open('../../frontend/src/pred.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)
